...
class CompassMoeConfig(PretrainedConfig):
    model_type = "compass_moe"
...
class CompassMoE(nn.Module):

    def __init__(
        self,
        model_config: ModelConfig[CompassMoeConfig],
        ...
        if self.use_shared_expert_gate == True:
...
class CompassMoeAttention(Attention):
...
class CompassMoeDecoderLayer(DecoderLayer):

    def __init__(self, model_config: ModelConfig[CompassMoeConfig],
                 layer_idx: int, aux_stream: torch.cuda.Stream):
        super().__init__()
        config = model_config.pretrained_config
        self.self_attn = CompassMoeAttention(
            model_config,
            layer_idx=layer_idx,
        )

        self.mlp = CompassMoE(model_config, aux_stream, layer_idx=layer_idx)
...
class CompassMoeModel(DecoderModel):
...
@register_auto_model("CompassMoeForCausalLM")
class CompassMoeForCausalLM(DecoderModelForCausalLM[CompassMoeModel,
                                                  CompassMoeConfig]):
    ...
    def __init__(
        ...
        super().__init__(CompassMoeModel(model_config),
                         config=model_config,
